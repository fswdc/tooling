...
**O que este comando faz**: exibe o caminho completo do diretório onde o shell está posicionado no momento.

**Por que ele existe**: em ambientes onde o prompt foi personalizado e não mostra o caminho, ou em scripts que precisam registrar o diretório de trabalho, `pwd` é a forma programática de obter essa informação.

**Decisão técnica**: `pwd` é um alias — um nome alternativo — para o cmdlet nativo do PowerShell `Get-Location`. O PowerShell 7 mantém `pwd` porque esse nome vem do Unix e é universal entre shells. Usar `pwd` em vez de `Get-Location` é idiomático e funciona no PowerShell, zsh e bash igualmente. No curso, usamos `pwd`.

**Onde é usado**: toda vez que for necessário confirmar onde o shell está antes de executar uma operação sensível à localização.
...
**O que este comando faz**: lista todos os arquivos e diretórios presentes no diretório atual.

**Por que ele existe**: navegar pelo sistema de arquivos sem conseguir ver o conteúdo de onde o shell está seria operar às cegas. `ls` é o equivalente em CLI de abrir uma pasta no Windows Explorer e ver os itens dentro dela.

**Decisão técnica**: `ls` é alias para o cmdlet nativo `Get-ChildItem`. O PowerShell 7 também aceita `dir`, que vem do Prompt de Comando legado. No curso usamos `ls` pelo mesmo motivo de `pwd`: é o nome universal entre shells, presente no PowerShell, zsh e bash. Aprender o alias universal reduz o custo de transição entre ambientes.

**Onde é usado**: toda vez que for necessário confirmar o conteúdo de um diretório antes de criar, mover ou deletar algo. Também usado para verificar se uma operação anterior produziu o resultado esperado — por exemplo, após criar um arquivo, `ls` confirma que ele está lá.
...
**O que este comando faz**: reposiciona o shell dentro do sistema de arquivos, tornando o diretório de destino o novo diretório atual. Todas as operações subsequentes passam a acontecer em relação ao novo local.

**Por que ele existe**: o shell sempre está em algum lugar. `cd` é o mecanismo de locomoção — o equivalente ao duplo clique em uma pasta no Windows Explorer.

**Decisão técnica**: `cd` é alias para o cmdlet `Set-Location`. É o comando de navegação universal — funciona com o mesmo nome no PowerShell, zsh e bash. Usaremos `cd` no curso por essa razão.

**Onde é usado**: toda vez que for necessário operar em um diretório diferente do atual. Em desenvolvimento, `cd` é usado dezenas de vezes por sessão de trabalho.
...
Todo usuário no Windows tem um diretório pessoal — o diretório inicial. No Windows, ele fica em `C:\Users\SeuNome`. É onde estão as pastas `Documents`, `Downloads`, `Desktop` e assim por diante.
...
**O que este comando faz**: leva o shell para o diretório inicial do usuário, independente de onde ele estiver no momento.

**Por que ele existe**: o diretório inicial é o ponto de referência central do usuário no sistema. Em vez de digitar o caminho completo — que varia de usuário para usuário e de máquina para máquina — `~` é uma referência portável que funciona em qualquer ambiente.

**Decisão técnica**: `~` é alias para a variável de ambiente `$HOME`. É o atalho de caminho universal — funciona com o mesmo nome no PowerShell, zsh e bash. No curso usaremos `~`.

**Onde é usado**: para retornar rapidamente ao ponto de partida, para referenciar o diretório inicial em scripts sem fixar o nome do usuário no código.
...
**O que este comando faz**: cria um novo diretório chamado `course` dentro do diretório atual.

**Por que ele existe**: toda estrutura de projeto começa com a criação de diretórios. Em desenvolvimento, código e configurações são organizados em pastas separadas. `mkdir` é o mecanismo de CLI para essa operação.

**Decisão técnica**: `mkdir` é alias para o cmdlet `New-Item -ItemType Directory`. O nome `mkdir` vem do Unix e é universal entre shells — funciona em PowerShell, zsh e bash com o mesmo nome. Usaremos `mkdir` no curso por essa razão.

**Onde é usado**: toda vez que for necessário criar uma nova pasta — ao iniciar um projeto, ao organizar subdiretórios, ao criar estruturas de pastas para demonstração.
...
**O que este comando faz**: cria um arquivo vazio chamado `notes.txt` no diretório atual.

**Por que ele existe**: em desenvolvimento, é frequente a necessidade de criar arquivos antes de preenchê-los — um arquivo de configuração, um arquivo de código. `New-Item` é o cmdlet nativo do PowerShell para essa operação.

**Decisão técnica**: no macOS e Linux, o comando equivalente é `touch notes.txt`. O PowerShell 7 não possui `touch` nativo. Essa é uma das divergências reais entre sistemas que foram antecipadas no Bloco 2. Em tutoriais escritos para macOS ou Linux, `touch` aparecerá com frequência. No ambiente Windows, o equivalente é `New-Item -ItemType File -Name`.

**Onde é usado**: para criar arquivos de configuração, arquivos de código inicialmente vazios. Toda vez que o arquivo precisa existir antes de ser preenchido.
...
**O que este comando faz**: escreve o texto `"First line of text."` dentro do arquivo `notes.txt`. Se o arquivo já tiver conteúdo, o texto é adicionado ao final — não substitui o que existe.

**Por que ele existe**: criar um arquivo vazio é apenas o primeiro passo. Em desenvolvimento, é frequente a necessidade de escrever conteúdo em arquivos diretamente pela CLI — configurações iniciais, conteúdo de demonstração — sem abrir um editor.

**Decisão técnica**: no macOS e Linux, a operação equivalente usa redirecionamento: `echo "text" >> file`. O `>>` redireciona a saída do comando `echo` para o arquivo, anexando ao final. O PowerShell 7 também aceita essa sintaxe, mas o comportamento do `echo` no PowerShell tem particularidades de codificação que podem gerar caracteres inesperados em alguns contextos. `Add-Content` é o cmdlet nativo e se comporta de forma previsível. No curso usaremos `Add-Content` no Windows por essa razão.

**Onde é usado**: para criar conteúdo inicial em arquivos de configuração, para adicionar linhas a arquivos de log em scripts, para qualquer situação em que seja necessário escrever em arquivo sem abrir um editor.
...
**O que este comando faz**: lê o conteúdo do arquivo `notes.txt` e exibe na tela, linha por linha.

**Por que ele existe**: após escrever em um arquivo, é necessário verificar o que está lá. Em desenvolvimento, arquivos de configuração e resultados de operações são lidos com frequência — tudo sem abrir um editor.

**Decisão técnica**: `cat` é alias para o cmdlet nativo do PowerShell `Get-Content`. No curso usaremos `cat` porque é mais rápido de digitar e é o nome que aparece em toda documentação e tutorial do ecossistema.

**Onde é usado**: para inspecionar arquivos de configuração, verificar o conteúdo de arquivos gerados. Toda vez que for necessário ver o conteúdo de um arquivo sem abrir um editor.
...
**O que este comando faz**: limpa todo o texto exibido na janela do terminal, deixando apenas o prompt vazio no topo.

**Por que ele existe**: após muitos comandos, a tela acumula saída e fica difícil de ler. `clear` reseta a visualização sem encerrar o shell nem alterar nenhum estado — o diretório atual, as variáveis e o histórico de comandos permanecem intactos.

**Decisão técnica**: `clear` é alias para o cmdlet nativo do PowerShell `Clear-Host`. O nome `clear` vem do Unix e é universal entre shells — funciona em PowerShell, zsh e bash com o mesmo nome. Usaremos `clear` no curso por essa razão.

**Onde é usado**: durante sessões longas de trabalho, quando a saída acumulada dificulta a leitura; antes de demonstrações em que se quer exibir apenas o resultado do próximo comando; em scripts que produzem saída estruturada e precisam de tela limpa como ponto de partida.
...
**O que este comando faz**: remove o diretório `course` e todo o seu conteúdo recursivamente — incluindo `notes.txt`.

**Por que ele existe**: ao trabalhar com demonstrações e experimentos em CLI, é necessário desfazer o que foi criado sem deixar resíduos no sistema de arquivos. `Remove-Item` é o cmdlet nativo do PowerShell para remoção de arquivos e diretórios.

**Decisão técnica**: as flags `-Recurse` e `-Force` são necessárias em conjunto para remover diretórios com conteúdo. Sem `-Recurse`, o PowerShell recusa remover um diretório que tem conteúdo dentro. Sem `-Force`, o PowerShell solicita confirmação interativa para cada item. No macOS e Linux, o equivalente é `rm -rf course` — mais conciso, mas com o mesmo comportamento destrutivo e irreversível.

**Onde é usado**: para limpar estruturas de diretório criadas em demonstrações, remover projetos descartados, ou desfazer experimentos de forma completa e não interativa.
